...
func create_pathfinding_points():
	...
	for i in range (0, 60):
		for j in range (0, 60):
			astar.add_point(point_id(Vector2(i, j)), Vector2(i, j)) 
...
#Adiciona unit à lista de units 
func add_unit(unit):
	units.append(unit)
...
func get_astar_path_avoiding_obstacles_and_units(start_position, end_position, exception_units = [], max_distance = -1):
	...
	var astar_path = astar.get_point_path(point_id(start_position), point_id(end_position))
...
func get_astar_path_avoiding_obstacles(start_position, end_position, max_distance = -1):
	set_obstacles_points_disabled(true)
	var potential_path_points = astar.get_point_path(point_id(start_position), point_id(end_position))
	set_obstacles_points_disabled(false)
...
func set_obstacles_points_disabled(value: bool):
	for obstacle in obstacles:
		astar.set_point_disabled(point_id(obstacle.global_position), value)
...
func set_unit_points_disabled(value: bool, exception_units: Array = []):
	for unit in units:
		if unit in exception_units or unit.owner in exception_units:
			continue
		astar.set_point_disabled(point_id(unit.global_position), value)
...
func get_floodfill_positions(start_position, min_range, max_range, skip_obstacles = true, skip_units = true, return_center = false):
	...
	while not checking_positions.empty():
		...
		var current_point = point_id(current_position)
		...
		for direction in DIRECTIONS:
			...
			var new_point = point_id(new_position)
...
func has_point(point_position):
	var point_id = point_id(point_position)
...
func get_used_cell_global_positions():
	...
	var cell_positions = []
	for cell in cells:
		var cell_position = global_position + cell
		cell_positions.append(cell_position)
	return cell_positions

func connect_cardinals(point_position):
	var center = point_id(point_position)
	for direction in DIRECTIONS:
		var cardinal_point = point_id(point_position + direction)
